...
class PatientAppointmentViewModel extends ChangeNotifier {
  Future<List<PatientAppointment>> getAllPatientAppointment(
      String userId) async {
    final response = await http.get(Uri.tryParse(
        '${url.BASE_URL}getAppointment?userId=$userId&userType=Doctor'));
    ...
    if (response.statusCode == 200) {
      ...
      patientAppointments =
          list.map((model) => PatientAppointment.fromJson(model)).toList();
