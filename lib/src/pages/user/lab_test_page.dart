...
class _LabTestPageState extends State<LabTestPage> {
                            ...
                            children: [
                              Expanded(
                                child: Text(
                                  '$day',
                                  style:
                                  TextStyle(color: ColorResources.black, fontSize: 14),
                                ),
                              ),
  ...
  void openDayPicker(BuildContext context) {
    ...
    DatePicker.showDatePicker(context,
        ...
        }, currentTime: DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day+1), locale: LocaleType.en);
  }
  ...
  void getLabTestById(BuildContext context,String id)async {
    List<LabTestByCategory> labtestByCat= await labTestViewModel.getLabTestByCategory(id);
  ...
  void saveDataIntoDb(BuildContext context)async {
    print('$testId $labtestId $id $amount $day $time');
    RegistrationResponse response = await labTestViewModel.saveLabTest(testId, labtestId, id,amount, day, time, 'Cash');
